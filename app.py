...
df = pd.read_csv('C:\INDIAN_STARTUP_EDA_STREAMLIT\startup_cleaned (1).csv')
...
def load_investor_details(investor):
    ...
    # Ensure 'year' and 'amount' are not null
    investor_df = investor_df.dropna(subset=['year', 'amount'])
    ...
    if not year_series.empty:
        ...
        ax2.set_xlabel('Year')
        ax2.set_ylabel('Total Amount (Cr)')
        ax2.set_title('YoY Funding Trend')
        st.pyplot(fig2)
    else:
        st.warning("No data available to plot Year-on-Year investment trend.")
